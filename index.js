...
class PackageRubyBundlePlugin {
  ...
  nativeLinuxBundle(){
    ...
    const result = execSync(`docker run --rm --volumes-from ${tempContainer} ${dockerImage} bundle config set path 'vendor/bundle' && bundle install --standalone --path vendor/bundle --redownload`)
    if (this.config.debug) {
      this.log(result)
    }
    execSync(`docker cp ${tempContainer}:/var/task/vendor/bundle ${localPath}/vendor`)
